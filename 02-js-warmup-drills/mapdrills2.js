...
console.log(getMaxTempByCity([{city:"van",temp:12},{city:"tor",temp:20},{city:"van",temp:15}]));
console.log(getMaxTempByCity([{city:"van",temp:12},{city:"tor",temp:20},{city:"van",temp:15}, {city:"van",temp:-15}]));
/**R6.** 
...
function isCountEqualsTovalues(nums) {
    ...
    const map = new Map(), set = new Set(nums);
    for(let n of nums) {
        map.set(n, (map.get(n) ?? 0) + 1)
    }
    for(const [k,v] of map) {
